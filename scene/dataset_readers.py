...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        bc_image_path = os.path.join(images_folder, 'base_color', os.path.basename(extr.name))
        bc_image = Image.open(bc_image_path)

        mro_image_path = os.path.join(images_folder, 'met_rough_occ', os.path.basename(extr.name))
...
def readSyntheticCamerasAndPoints(storage_path, file_name, extension, metadata_file):
    ...
    with open(os.path.join(storage_path, metadata_file)) as metadata_json:
        ...
        for idx, sample in enumerate(content["cameras"]):
            ...
            normal_image_path = os.path.join(storage_path, data_types[3], image_name)
            normal_image = Image.open(normal_image_path)
